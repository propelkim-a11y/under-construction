...
function drawScene() {
 ...
 return { x: (dprWidth / 2) + (pZ * targetViewScale), y: (dprHeight * 0.6) - (pY * targetViewScale) };
 }
 return { x: 0, y: 0 };
 }
 ...
 // 4. 과녁 확대도 (Target View) 격자 렌더링
 } else if (currentView === 'target') {
 // [요청 반영] 바닥 기준선을 화면 60% 비율(0.6) 위치로 안전하게 고정
 ...
 const thickX = TGT_D * Math.cos(TGT_TILT); const thickY = -TGT_D * Math.sin(TGT_TILT);
 const bBottom = toScreen(targetBaseX + thickX, safeTargetH + thickY, 0); 
 const bTop = toScreen(frontTopX + thickX, frontTopY + thickY, 0);

 ctx.fillStyle = '#e5e5ea'; ctx.beginPath(); ctx.moveTo(fBottom.x, fBottom.y); ctx.lineTo(fTop.x, fTop.y); ctx.lineTo(bTop.x, bTop.y); ctx.lineTo(bBottom.x, bBottom.y); ctx.closePath(); ctx.fill();
 ctx.strokeStyle = '#1d1d1f'; ctx.lineWidth = 1; ctx.stroke();
 ctx.strokeStyle = '#1d1d1f'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(fBottom.x, fBottom.y); ctx.lineTo(fTop.x, fTop.y); ctx.stroke();
 ...
 const w = rightX - leftX; const h = bottomY - topY;
 ...
 const projTopX = targetBaseX + TGT_H * Math.sin(TGT_TILT); const thickX = TGT_D * Math.cos(TGT_TILT);
 const fLeftBot = toScreen(targetBaseX, safeTargetH, -TGT_W / 2); const fRightBot = toScreen(targetBaseX, safeTargetH, TGT_W / 2);
 const bLeftTop = toScreen(projTopX + thickX, safeTargetH, -TGT_W / 2); const bRightTop = toScreen(projTopX + thickX, safeTargetH, TGT_W / 2);
 ...
 const tBottomY = dprHeight * 0.6; 
 const tLeftX = (dprWidth / 2) - (TGT_W / 2 * targetViewScale);
 const tRightX = (dprWidth / 2) + (TGT_W / 2 * targetViewScale);
 const tTopY = tBottomY - (VISUAL_TGT_PROJ_H * targetViewScale);
 const w = tRightX - tLeftX; const h = tBottomY - tTopY; 
